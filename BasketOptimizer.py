...
''' 
	BasketOptimizer = extension of the Optimizer that handles a basket of names.
	 = strategy is run against data for each name in the basket of securities

	Steps:
	1. Run all names under one parameter set P(x,y): results = R1 for name1, R2 for name2, etc.  
	2. Come up with an aggregate score for R1, R2, R3, ... Rn, => S1.
	3. Repeat for new P(x1, y1), until you generate enough scores to fill the population.  (S1, S2, ... Sn)
	4. Then go GA Optiimization as noted.

	Development strategy:
	A. First verison with collect all the scores serially.
	- aggregate scoring function take the standard scoring function S,
         and computes:
           aggregate scoring function X = (number of tests that exceed min trade count)/basket_count) * average(S')/stdev(S')
           where S' is the array of all scores for each name in the basket = [S1, S2, .. Sn],
           where basket_count = n (number of names in the basket)

	B. Need to employ celery / rabbitmq to create a distributed version
    ...
    ## add an array that describe the basket:
    ## one DataFeed object for each name in the basket
    optimizer.data_basket = [DataFeedDaily('USD.csv'),DataFeedDaily('HKD.csv')]
     -- OR --
    optimizer.data_basket = [DataFeedList(['SPY1.csv','SPY2.csv'],data_type='B'),DataFeedList(['QQQ1.csv','QQQ2.csv'],data_type='B')]
    ## set population size
    optimizer.size = 40 
...
class BasketOptimizer(object):
	...
	## iterate over list of scores and produce composite score
	def basket_score(self,score_list,run_id):

		def fix_nulls(x):
			if type(x) in [int, long, float]: return float(x)
			return numpy.nan 

		composite = []    
		for score_data in score_list:
			score_input, filename = score_data
			log.info('component score data %s: %s' % (filename,score_input))
			score_input['_score'] = fitness_function(score_input, min_trades=self.min_trade_count)
			composite.append(score_input)


		cdf = pandas.DataFrame(composite)
		cdf = cdf.applymap(fix_nulls)
		valids = cdf[cdf['cnt'] >= self.min_trade_count]

		## flatten all rows to a single mean value per stat
		composite = dict(cdf.mean())
		composite['_score'] = 0
		## grb summary stats for all valid test (where trade count > min trade count)
		if valids.shape[0] > 0:
			composite = dict(valids.mean())
			## weight by percentage of valids to total basket size
			composite['_score'] *= (valids.shape[0]/float(len(score_list)))	
	
		## self.population = [] of
		## [fitness_score, strand_bit_string] elements
		self.population[self.index][0] = composite['_score']
		...
		self.results_map[bitcode] = (composite, run_id)
	...
	def run(self):
	    ...
	    while not self.converged():
			...
			for strategy_params in params_set:
				...
				## collect summary for each data_set - as they are separated for each name in the data_basket
				summaries =[]
				for data_feed in self.data_basket:
					s = Simulator()
					...
					data_feed.reset()
					## make this run call distributed - then parallelized
					summary = s.run(data_feed)
					summaries.append((summary,data_feed.filename))
				## take all the summaries and come up with a basket score
				self.basket_score(summaries,run_id)
				run_id += 1
